...
class LookAdd extends Component {
  ...
  onAddPressed = (context, gallery) => {
    this.props.setContext(context)
    if(gallery) {
      // Actions.productForm()
      Actions.recordMenu({component:'GALLERY', gallery:true})
    } else {
      Actions.recordMenu({component:'CAMERA', gallery:false})
    }
  }
  ...
  render = () => {
    return(
      <View style={{width, height: width > 600 ? (height * 0.9) : (height * 0.8), backgroundColor: '#F8F8F8', alignItems: 'center', justifyContent: 'center'}}>
        ...
        <TouchableWithoutFeedback onPress={() => this.onAddPressed('LOOK', true)}>
