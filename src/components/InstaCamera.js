...
class InstaCamera extends Component {
  ...
  onSnapTaken = () => {
    ...
    if(this.camera){
      ...
      this.camera.takePictureAsync({ quality: 0.5 })
        .then(fileUri => {
          console.log("Success " + fileUri)
          this.props.onPhotoSuccess(fileUri, this.props.context)
        })
        .catch(error => {
          console.log("Error fetching camera: " + error)
          this.props.onPhotoError(error)
        })
    }
  }
  ...
  render = () => {
    ...
    if(hasPermission === null){
      ...
      return <Text>No permission for access to camera</Text>
    } else {
      return(
