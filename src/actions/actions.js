...
sendToAWS = async (URLString, s3Key, callback) => {
  try {
    ...
    AWS.config.update({
      region: 'us-east-1',
      accessKeyId: 'AKIAJI523WAIWREMSM7Q',
      secretAccessKey: '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'
    })
    ...
    let imageUrl = `https://s3.us-east-1.amazonaws.com/instalookmua/${s3Key}.jpg`
  ...
  return dispatch => {
    ...
    firebase.auth().createUserWithEmailAndPassword(email, password)
      .then(FBUser => {
        ...
        let user = { uid: FBUser.user.uid, displayName: userName, firstName, lastName, email: FBUser.user.email }
...
export const onPhotoError = (error) => {
  ...
  return { type: types.PHOTO_TAKEN_ERROR, error }
}
...
export const getProductBy = (id) => {
  return (dispatch) => {
    ...
    getProductOnServer(dispatch, id)
      ...
      .catch(error => {
        console.log('Error from products by: ' + error)
...
export const sendComment = payload => {
  return async dispatch => {
    try {
      console.log("THIS_IS_THE_PAYLOAD " + JSON.stringify(payload))
      console.log("SENDING COMMENT")
      dispatch({ type: types.SEND_COMMENT })
      let result = await sendCommentOnServer(payload)
      console.log("SENDING COMMENT " + JSON.stringify(result))
...
export const changePostDisplayedTo = type => {
  return { type: types.CHANGE_POST_TYPE_DISPLAYED, payload: type }
}

//HELPERS

export const appGetCredentialsOnServer = user => {
 return async dispatch => {
  let result = await getCredentialsOnServer(user)
  console.log(result)
  dispatch({ type: types.USER_PROFILE, payload: result.data })
 }
}
...
const loginUserSuccess = async (dispatch, user) => {
  try {
    ...
    console.log("CATCHED_HERE " + error)
  }
}
...
const isABanishedUser = async user => {
  try {
    ...
    Alert.alert('Some error occured, please login again.')
  }
}
